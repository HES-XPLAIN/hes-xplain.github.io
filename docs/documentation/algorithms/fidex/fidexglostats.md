...
`Train Accuracy`
: The accuracy of the rule on the samples it covers. In the case of Rule 1, 96.43% of the covered samples are correctly classified.

`Train Confidence`
: Represents the average confidence score of the model’s predictions for the samples covered by the rule. For Rule 1, the confidence is 97.12%.
